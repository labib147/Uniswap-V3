...
const extractLiveQueryRootFieldCoordinates = (params) => {
    ...
    graphql.visit(params.documentNode, graphql.visitWithTypeInfo(params.typeInfo, {
        Field(fieldNode) {
            var _a;
                    ...
                    for (const arg of fieldDef.args) {
                        if (arg.name === "id") {
                            ...
                            if (fieldSDLType === "ID!" || fieldSDLType === "ID") {
                                const values = values_js.getArgumentValues(fieldDef, fieldNode, params.variableValues);
...
const isNonNullIDScalarType = (type) => {
    if (graphql.isNonNullType(type)) {
        return graphql.isScalarType(type.ofType) && type.ofType.name === "ID";
    }
    return false;
};
...
const addResourceIdentifierCollectorToSchema = (schema, idFieldName) => utils.mapSchema(schema, {
    [utils.MapperKind.OBJECT_FIELD]: (fieldConfig, fieldName, typename) => {
        ...
        let resolve = (_a = fieldConfig.resolve) !== null && _a !== void 0 ? _a : graphql.defaultFieldResolver;
...
class InMemoryLiveQueryStore {
    constructor(params) {
        ...
        this.makeExecute = (execute) => (args) => {
            ...
            const liveDirectiveNode = graphqlLiveQuery.getLiveDirectiveNode(operationNode);
            if (isNone(liveDirectiveNode)) {
                return fallbackToDefaultExecute();
            }
            let { isLive, throttleValue } = graphqlLiveQuery.getLiveDirectiveArgumentValues(liveDirectiveNode, variableValues);
            if (isLive === false) {
                return fallbackToDefaultExecute();
            }
            if (isSome(this._validateThrottleValue)) {
                ...
                if (typeof maybeErrorOrNewThrottleValue === "string") {
                    return {
                        errors: [
                            new graphql.GraphQLError(maybeErrorOrNewThrottleValue, [
                                liveDirectiveNode,
                            ]),
                        ],
                    };
