...
import { defaultImportFn, loadYaml, writeFile, pathExists, writeJSON, DefaultLogger, loadFromModuleExportExpression, stringInterpolator, rmdirs } from '@graphql-mesh/utils';
import Ajv from 'ajv';
...
import { print, Kind, printSchema } from 'graphql';
import { codegen } from '@graphql-codegen/core';
import { pascalCase } from 'pascal-case';
import { getRootTypeMap, buildOperationNodeForField, parseGraphQLSDL, printSchemaWithDirectives, inspect } from '@graphql-tools/utils';
...
import { exit, env, on, cwd } from 'process';
...
function customLoader(ext, importFn = defaultImportFn) {
...
export async function ${cliParams.builtMeshSDKFactoryName}<TGlobalContext = any, TOperationContext = any>(globalContext?: TGlobalContext) {
  ...
  return getSdk<TOperationContext>(sdkRequesterFactory(globalContext));
    ...
    const baseUrlAssignmentESM = `const baseDir = pathModule.join(pathModule.dirname(fileURLToPath(import.meta.url)), '${path.relative(artifactsDir, baseDir)}');`;
    const baseUrlAssignmentCJS = `const baseDir = pathModule.join(__dirname, '${path.relative(artifactsDir, baseDir)}');`;
...
const graphqlHandler = (mesh$) => {
    const yoga$ = mesh$.then(mesh => createServer({
        parserCache: false,
...
function handleFatalError(e, logger = new DefaultLogger('🕸️')) {
...
function registerTerminateHandler(callback) {
    for (const eventName of terminateEvents) {
        on(eventName, () => callback(eventName));
    }
}
async function serveMesh({ baseDir, argsPort, getBuiltMesh, logger, rawConfig, playgroundTitle }, cliParams) {
    var _a, _b;
    const { fork, port: configPort, hostname = platform() === 'win32' ? 'localhost' : '0.0.0.0', cors: corsConfig, handlers, staticFiles, playground, maxRequestBodySize = '100kb', sslCredentials, endpoint: graphqlPath = '/graphql', browser, } = rawConfig.serve || {};
    ...
    if (!cluster.isWorker && Boolean(fork)) {
        ...
        if (sslCredentials) {
            ...
            httpServer = createServer$1({ key, cert }, app);
        }
        else {
            httpServer = createServer$2(app);
        }
        registerTerminateHandler(eventName => {
            const eventLogger = logger.child(`${eventName}💀`);
        ...
        await Promise.all((handlers === null || handlers === void 0 ? void 0 : handlers.map(async (handlerConfig) => {
            ...
            if ('handler' in handlerConfig) {
                handlerFn = await loadFromModuleExportExpression(handlerConfig.handler, {
                    cwd: baseDir,
        ...
        if (typeof playground !== 'undefined' ? playground : ((_b = env.NODE_ENV) === null || _b === void 0 ? void 0 : _b.toLowerCase()) !== 'production') {
        ...
        httpServer
            ...
            var _a;
            const shouldntOpenBrowser = ((_a = env.NODE_ENV) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === 'production' || browser === false;
...
async function graphqlMesh(cliParams = DEFAULT_CLI_PARAMS, args = hideBin(process.argv), cwdPath = cwd()) {
    let baseDir = cwdPath;
    ...
    return yargs(args)
        ...
        try {
            ...
            meshInstance$
                .then(({ schema }) => writeFile(path.join(outputDir, 'schema.graphql'), printSchema(schema)))
                .catch(e => {
        ...
        try {
            ...
            if ((_a = rawConfig.serve) === null || _a === void 0 ? void 0 : _a.customServerHandler) {
                const customServerHandler = await loadFromModuleExportExpression(rawConfig.serve.customServerHandler, {
                    defaultExportName: 'default',
                    cwd: baseDir,
                    importFn: defaultImportFn,
                });
        ...
        try {
            ...
            await writeFile(path.join(outputDir, 'schema.graphql'), printSchema(schema));
