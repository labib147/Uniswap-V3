...
async function resolveLogger(loggerConfig, importFn, cwd) {
    ...
    const logger = new utils$1.DefaultLogger('🕸️');
    return {
        logger,
...
async function processConfig(config, options) {
    ...
    const { dir, importFn = utils$1.defaultImportFn, store: providedStore, artifactsDir } = options || {};
    if (config.require) {
    ...
    for (const additionalResolverDefinitionIndex in config.additionalResolvers) {
        ...
        if (typeof additionalResolverDefinition === 'string') {
            importCodes.push(`import * as additionalResolvers$${additionalResolverDefinitionIndex} from '${crossHelpers.path
    ...
    if (config.additionalEnvelopPlugins) {
        importCodes.push(`import importedAdditionalEnvelopPlugins from '${crossHelpers.path
