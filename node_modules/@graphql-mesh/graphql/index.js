...
class GraphQLHandler {
    ...
    getCustomFetchImpl(customFetchConfig) {
        return customFetchConfig
            ? utils.loadFromModuleExportExpression(customFetchConfig, {
                cwd: this.baseDir,
                defaultExportName: 'default',
                importFn: this.importFn,
            })
            : utils.getCachedFetch(this.cache);
    }
    async getExecutorForHTTPSourceConfig(httpSourceConfig) {
        ...
        const operationHeadersFactory = utils.getInterpolatedHeadersFactory(operationHeaders);
    ...
    async getNonExecutableSchemaForHTTPSource(httpSourceConfig) {
        const schemaHeadersFactory = utils.getInterpolatedHeadersFactory(httpSourceConfig.schemaHeaders || {});
        ...
        if (httpSourceConfig.introspection) {
            ...
            throw new Error(`Invalid introspection data: ${utils$1.inspect(sdlOrIntrospection)}`);
        }
        return this.nonExecutableSchema.getWithSet(() => {
            const endpointFactory = utils.getInterpolatedStringFactory(httpSourceConfig.endpoint);
    ...
    async getCodeFirstSource({ schema: schemaConfig, }) {
        if (schemaConfig.endsWith('.graphql')) {
            ...
            if (schemaOrStringOrDocumentNode instanceof graphql.GraphQLSchema) {
                ...
                throw new Error(`Provided file '${schemaConfig} exports an unknown type: ${utils$1.inspect(schemaOrStringOrDocumentNode)}': expected GraphQLSchema, SDL or DocumentNode.`);
            }
            return {
                schema,
            };
    ...
    async getMeshSource() {
        ...
        throw new Error(`Unexpected config: ${utils$1.inspect(this.config)}`);
    }
}
